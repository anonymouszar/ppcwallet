...
type BlockMeta struct {
	Block
	Time                time.Time
	KernelStakeModifier uint64 // ppc:
}
...
func (s *Store) unspentOutputs(ns walletdb.Bucket) ([]Credit, error) {
	...
	err := ns.Bucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		blockKernelStakeModifier, err :=
			fetchBlockKernelStakeModifier(ns, block.Height) // ppc:
		if err != nil {
			return err
		}
...
func (s *Store) Balance(minConf, syncHeight int32,
	...
	err := scopedView(s.namespace, func(ns walletdb.Bucket) error {
		var err error
		amt, err = s.balance(ns, minConf, syncHeight, net)
		return err
	})
	return amt, err
}
...
func (s *Store) balance(ns walletdb.Bucket, minConf int32, syncHeight int32,
	...
	stopConf := minConf
	if int32(net.CoinbaseMaturity) > stopConf {
		stopConf = int32(net.CoinbaseMaturity)
	}
	...
	for blockIt.prev() {
		...
		for i := range block.transactions {
			...
			for i := uint32(0); i < numOuts; i++ {
				...
				if confs < minConf || (blockchain.IsCoinStakeTx(&rec.MsgTx) && // ppc:
					confs < int32(net.CoinbaseMaturity)) { // ppc:
					bal -= amt
				}
			}
		}
