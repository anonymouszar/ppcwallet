...
//   [40:48] Kernel stake modifier (8 bytes) (ppc:)
//   [48:52] Number of transaction hashes (4 bytes) (ppc:)
//   [52:]   For each transaction hash: (ppc:)
//             Hash (32 bytes)
...
func valueBlockRecord(block *BlockMeta, txHash *wire.ShaHash) []byte {
	...
	byteOrder.PutUint64(v[40:48], block.KernelStakeModifier) // ppc:
	byteOrder.PutUint32(v[48:52], 1)                         // ppc:
	copy(v[52:84], txHash[:])                                // ppc:
	return v
}
...
func appendRawBlockRecord(v []byte, txHash *wire.ShaHash) ([]byte, error) {
	...
	n := byteOrder.Uint32(newv[48:52])    // ppc:
	byteOrder.PutUint32(newv[48:52], n+1) // ppc:
	return newv, nil
}
...
func fetchBlockTime(ns walletdb.Bucket, height int32) (time.Time, error) {
	...
	if len(v) < 52 { // ppc:
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketBlocks, 52, len(v)) // ppc:
		return time.Time{}, storeError(ErrData, str, nil)
	}
	return time.Unix(int64(byteOrder.Uint64(v[32:40])), 0), nil
}

func fetchBlockKernelStakeModifier(ns walletdb.Bucket, height int32) (uint64, error) {
	k := keyBlockRecord(height)
	v := ns.Bucket(bucketBlocks).Get(k)
	if len(v) < 52 { // ppc:
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketBlocks, 52, len(v)) // ppc:
		return uint64(0), storeError(ErrData, str, nil)
	}
	return byteOrder.Uint64(v[40:48]), nil
}
...
func readRawBlockRecord(k, v []byte, block *blockRecord) error {
	...
	numTransactions := int(byteOrder.Uint32(v[48:52])) // ppc:
	expectedLen := 52 + wire.HashSize*numTransactions
	...
	block.KernelStakeModifier = byteOrder.Uint64(v[40:48]) // ppc:
