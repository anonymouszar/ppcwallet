...
func GetTransaction(w *wallet.Wallet, chainSvr *chain.Client, icmd interface{}) (interface{}, error) {
	...
	ret := btcjson.GetTransactionResult{
		...
		Time:            details.MsgTx.Time.Unix(),
		TimeReceived:    details.MsgTx.Time.Unix(),
	...
	credCat := wallet.RecvCategory(details, syncBlock.Height, w.Manager.ChainParams()).String()
	for _, cred := range details.Credits {
		// Change is ignored.
		if cred.Change {
			continue
		...
		_, addrs, _, err := txscript.ExtractPkScriptAddrs(
			details.MsgTx.TxOut[cred.Index].PkScript, w.Manager.ChainParams())
